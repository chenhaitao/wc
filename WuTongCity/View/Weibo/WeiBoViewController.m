...
- (void)viewDidLoad{
    ...
    self.messageView = [[UIView alloc] initWithFrame:CGRectMake(0, -34, 320, 34)];
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.frame = CGRectMake(0, 0, 320, 34);
    ...
    [button setTitle:@"您有新评论，请注意查收！" forState:UIControlStateNormal];
    button.titleLabel.font = [UIFont systemFontOfSize:12.0f];
    [button addTarget:self action:@selector(newCommentAction) forControlEvents:UIControlEventTouchUpInside];
...
{
    [[NSUserDefaults standardUserDefaults] setBool:NO forKey:@"kCommentMessageFlag"];
    NewWeiboCommentViewController *nwb = [[NewWeiboCommentViewController alloc] init];
    [nwb setDataSource:newWeiboComment];
    [self.navigationController pushViewController:nwb animated:YES];
}
        ...
        if (respString.length > 0) {
            ...
            if ([respString JSONValue]) {
                ...
                for (NSDictionary *topicDict in reqArray) {
                    ...
                    [newWeiboComment addObject:topicVO];
                }
                
                
                //排序按照时间
                [newWeiboComment sortUsingComparator: ^(TopicVO *s1, TopicVO *s2){
                    return [s2.createTime compare:s1.createTime];
                }];
                
                if (newWeiboComment.count) {
...
- (void)showWeiboCommentMessage
{
    ...
    if (self.view.window == nil) {
        ...
        [UIView animateWithDuration:0.5 animations:^{
            self.messageView.transform = CGAffineTransformMakeTranslation(0, 36);
        } completion:^(BOOL finished) {
            [UIView  animateWithDuration:0.3 animations:^{
                self.messageView.transform = CGAffineTransformTranslate(self.messageView.transform, 0, -2);
            }];
        }];
    }
...
//    [self showRefreshHeader:YES];
    ...
    BOOL flag = [[NSUserDefaults standardUserDefaults] boolForKey:@"kCommentMessageFlag"];
    if (flag) {
        [self.navigationController.tabBarItem setBadgeValue:nil];
        [UIView animateWithDuration:0.5 animations:^{
            self.messageView.transform = CGAffineTransformMakeTranslation(0, 36);
